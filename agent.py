...
from middleware.dynamic_system_prompt import dynamic_system_prompt, dynamic_system_prompt_eco
...
def create_email_agent(model_name: str = "mistral-large-latest", eco: bool = False):
    ...
    return create_agent(
        model=model, 
        tools=get_tools(eco=eco), 
        state_schema=State, 
        context_schema=Context, 
        middleware=[trim_messages, 
                    clear_tool_outputs, 
                    *([] if eco else [judge_current_step]), 
                    dynamic_system_prompt_eco if eco else dynamic_system_prompt, 
                    fallback],
        debug=True
    )